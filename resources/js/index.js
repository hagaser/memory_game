...
window.onload = function() { // when the page loads
  ...
  if (numberOfSlider) { //checking for a slider
    ...
    if (window.location.href.replace(/\D/g, '')) { // if the game is started

      const observer = new MutationObserver((mutationsList) => { // observing for changing

        for(let mutation of mutationsList) {
          if (mutation.type === 'childList') {

            if (!document.getElementsByClassName('img-div')
                .length) { // if there are no cards

              document.getElementById('win')
              .style.display = 'inline-block';

              document.getElementById('game_block')
              .style.display = 'none';
